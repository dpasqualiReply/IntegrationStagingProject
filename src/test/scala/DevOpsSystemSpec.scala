...
//      }
  ...
  override def afterAll(): Unit = {
    ...
    if(toggleRTETL) {
      ...
      storage.deleteKuduRows(storage.readKuduTable(s"$kuduDB.$kTags").select("id"), s"$kuduDB.$kTags")
      storage.deleteKuduRows(storage.readKuduTable(s"$kuduDB.$kRatings").select("id"), s"$kuduDB.$kRatings")
    }
