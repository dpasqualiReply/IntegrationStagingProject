...
//    }
  ...
  override def afterAll(): Unit = {
    ...
    if(toggleRTETL) {
      ...
      storage.deleteKuduRows(storage.readKuduTable(s"$kuduDB.$kTags").select("tagid"), s"$kuduDB.$kTags")
      storage.deleteKuduRows(storage.readKuduTable(s"$kuduDB.$kRatings").select("movieid"), s"$kuduDB.$kRatings")
    }
